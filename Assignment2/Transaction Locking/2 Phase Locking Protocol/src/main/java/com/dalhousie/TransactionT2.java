...
public class TransactionT2 extends Thread implements MyTransaction{
  
    public HashMap<String,String> transLockTables = new HashMap<String,String>();
   
	public TransactionT2() {
	...
	public boolean operation(Connection conn) {
		...
		try {
			...
			db.execute(conn, db.getClassIdQuery("Auditorium")); 
			db.executeUpdate(conn, db.getUpdateCourseClassQuery(5408)); 
			db.executeUpdate(conn, db.getDeleteClassroomQuery("Tupper Theater"));
			db.executeUpdate(conn, db.getUpdateBookPriceQuery(101,9));
